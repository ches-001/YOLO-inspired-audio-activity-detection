...
class AudioDetectionLoss(nn.Module):
    ...
    def forward(
        ...
        metrics_dict["avg_pos_conf"] = metrics_df["avg_pos_conf"].mean()
        metrics_dict["avg_neg_conf"] = metrics_df["avg_neg_conf"].mean()
    ...
    def loss_fn(
        ...
        conf_loss = (0.9 * conf_loss) + (0.05 * neg_conf.pow(2).mean()) + (0.05 * (ciou - pos_conf).pow(2).mean())
